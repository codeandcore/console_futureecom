<template>
  <grid-layout v-bind="{ grid, params, options, title: $t('Sale taxes') }" @paramsChanged="reloadCollection">
    <template #actions>
      <v-btn v-if="canCreate" data-cy="add-new-tax-rate" depressed color="primary" :to="{ name: 'sale-tax-create' }">
        {{ $t('Add new sale tax') }}
      </v-btn>
    </template>
    ...
    <template #item.edit="{ item }">
      <button-action
        ...
        :to="{ name: 'sale-tax-update', params: { id: item.id } }"
      />
    </template>
    <template #item.delete="{ item }">
      <sale-tax-action-delete icon :value="item" @success="onDeleteSuccess" @error="onRequestError" />
    </template>
  </grid-layout>
</template>
...
import type { TaxRatesFilter, TaxRatesGridRequest } from '@futureecom/futureecom-js/dist/services/tax-service'
...
export default defineComponent({
  ...
  setup() {
    ...
    const params = useQueryFromUrl<TaxRatesGridRequest>({ sort: ['-created_at'] })
    ...
    const reloadCollection = (gridParams?: TaxRatesGridRequest): void => {
      params.value = { ...params.value, ...gridParams }
      grid.loading = true

      api.taxes
        .taxRates(params.value)
        .then((value) => {
    ...
    const options = computed((): ResourceGridOptions => {
      ...
      const filters: FilterDefinition<TaxRatesFilter>[] = saleTaxFilters.map((filter) => ({
        ...filter,
