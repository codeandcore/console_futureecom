  ...
  <form-validator-wrapper @submit="createHandler">
    <template #default="{ valid }">
      <scrollable-resource class="d-flex flex-column">
        <template #header>
          <resource-header :back-to="redirectBack" :header="$t('Add new preset page')">
            <template #actions>
...
import type { NewPresetPage } from '@futureecom/futureecom-js/dist/services/cms-service'

export default defineComponent({
  name: 'PresetPageCreate',
  components: {
    ResourceHeader,
  ...
  setup() {
    ...
    const createHandler = () => {
      ...
      api.cms
        .createPresetPage(route.params.preset, item.value)
        .then(({ id }) => {
          addSuccessToast(t("Created preset's page"))
          router.push({ name: 'preset-page-update', params: { preset: route.params.preset, id } }).catch()
        })
        .catch((err) => addErrorToast(err.response?.data.message || t("Unable to create preset's page")))
