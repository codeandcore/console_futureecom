<template>
  <div class="d-flex flex-column flex-gap-16">
    <v-text-field
      v-model="searchVal"
      outlined
      :label="$t('Search permission name')"
      append-icon="search"
      hide-details
    />
...
import type { PermissionGridRequest } from '@futureecom/futureecom-js/dist/services/auth-service'
...
export default defineComponent({
  name: 'SelectMultiplePermissions',
  ...
  props: {
    ...
    options: {
      type: Object as PropType<SelectPermissionOptions>,
  ...
  setup(props, { emit }) {
    const searchVal = ref('')
    const currentParams = ref<PermissionGridRequest>()
    ...
    const reloadCollection = (params: PermissionGridRequest): void => {
      currentParams.value = params
      collection.loadCollection(() => {
        return api.auth.permissions({ ...currentParams.value, filter: { name: searchVal.value || undefined } })
      })
    }
