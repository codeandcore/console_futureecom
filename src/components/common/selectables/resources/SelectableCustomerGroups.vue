...
export default defineComponent({
  ...
  props: {
    value: {
      type: Object as PropType<{ customer_group?: null | string | string[] }>,
      default: undefined
    },
    items: {
  ...
  setup(props, { emit }) {
    const { canRead } = useCrudChecker('customers\\group')
    ...
    const fetchSelectedCodes = () => {
      const codes = Array.isArray(props.value?.customer_group)
        ? (props.value?.customer_group || []).filter(Boolean)
        : [props.value?.customer_group || ''].filter(Boolean)
