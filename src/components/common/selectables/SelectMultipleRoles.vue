<template>
  <div class="d-flex flex-column flex-gap-16">
    <v-text-field v-model="searchVal" outlined :label="$t('Search role name')" append-icon="search" hide-details />
...
import type { Role, RoleGridRequest } from '@futureecom/futureecom-js/dist/services/auth-service'
...
export default defineComponent({
  ...
  props: {
    ...
    options: {
      type: Object as PropType<SelectRoleOptions>,
  ...
  setup(props, { emit }) {
    const searchVal = ref('')
    const currentParams = ref<RoleGridRequest>()
    ...
    const reloadCollection = (params: RoleGridRequest): void => {
      currentParams.value = params
      collection.loadCollection(() => {
        return api.auth.roles({ ...currentParams.value, filter: { name: searchVal.value || undefined } })
      })
    }
