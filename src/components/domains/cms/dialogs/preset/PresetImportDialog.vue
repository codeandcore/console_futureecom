...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const importHandler = (): void => {
      ...
      if (data.value.file) {
        formData.append('preset', data.value.file)
      }

      dataParts.forEach((part): void => formData.append('parts[]', part))

      api.cms
        .importPreset(formData)
        .then(() => {
          showDialog.value = false
          addSuccessToast(t('Preset imported'))
          emit('onSuccess')
        })
        .catch(() => addErrorToast(t('Unable to import preset')))
