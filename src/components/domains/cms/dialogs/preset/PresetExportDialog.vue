        ...
        <v-card>
          <v-card-title class="d-flex justify-space-between">
            <div class="d-flex flex-column">
              <span>{{ $t('Export preset') }}: {{ value.name }}</span>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const exportHandler = (): void => {
      ...
      api.cms
        .exportPreset(props.value.code, { parts: parts.value.length ? parts.value : ['base'] })
        .then((val) => {
          const fileData = [JSON.stringify(val)]
          const fileName = `${props.value.code}_preset.json`
          createLink(fileData, fileName).click()
        })
        .catch(() => addErrorToast(t('Unable to export presets')))
