...
export default defineComponent({
  name: 'VatRateActionDelete',
  components: { DeleteButton },

  props: {
    value: {
      type: Object as PropType<VatRate>,
      required: true
    },
    icon: {
      type: Boolean,
  ...
  setup(props, { emit }) {
    ...
    const deleteHandler = () => {
      ...
      api.taxes
        .deleteVatRate(props.value.id)
        .then(() => {
          addSuccessToast(t('Deleted vat rate'))
          emit('success')
        })
        .catch((err) => {
          addErrorToast(t('Unable to delete vat rate'))
          emit('error', err)
        })
        .finally(() => {
          loading.value = false
