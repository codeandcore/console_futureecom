<template>
  <v-text-field v-model="name" v-bind="$attrs" :label="$t('Name')" outlined hide-details="auto" />
</template>
...
export default defineComponent({
  name: 'GeneralSettingContactAddressName',
  ...
  setup(props, { emit }) {
    const name = computed({
      ...
      set: (name) => emit('input', { ...props.value, name })
    })
