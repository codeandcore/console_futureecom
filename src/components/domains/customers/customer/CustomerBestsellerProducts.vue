...
export default defineComponent({
  name: 'CustomerBestsellerProducts',
  ...
  setup(props, { emit }) {
    ...
    if ((props.value.bestseller_products || []).length) {
      api.catalog
        .products({ filter: { id: props.value.bestseller_products } })
        .then((val) => (products.value = val.data))
    }

    const bestseller_products = computed({
      get: () => props.value.bestseller_products || [],
      set: (bestseller_products) => emit('input', { ...props.value, bestseller_products })
    })
    ...
    const onSelect = (val: Product[]) => {
      products.value = val
      bestseller_products.value = val.map((item) => item.id)
