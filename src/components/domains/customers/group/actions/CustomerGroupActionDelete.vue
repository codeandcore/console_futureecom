...
export default defineComponent({
  name: 'CustomerGroupActionDelete',
  components: { DeleteButton },

  props: {
    value: {
      type: Object as PropType<CustomerGroup>,
      required: true
    },
    icon: {
      type: Boolean,
  ...
  setup(props, { root, emit }) {
    ...
    const deleteHandler = () => {
      ...
      api.customers
        .deleteGroup(props.value.id)
        .then((val) => {
          root.$toast.addSuccessToast('Deleted customer group')
          emit('onSuccess', val)
        })
        .catch((err) => {
          root.$toast.addErrorToast('Unable to delete customer group')
          emit('onError', err)
        })
        .finally(() => {
          loading.value = false
