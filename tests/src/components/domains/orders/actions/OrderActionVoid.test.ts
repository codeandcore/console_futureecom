...
const attributes = {
  component: 'order-action-void-btn',
  yesBtn: 'order-action-void-yes-btn',
  noBtn: 'order-action-void-no-btn',
...
describe('OrderActionVoid input component test', () => {
  ...
  it('should mount OrderActionVoid component correctly', () => {
    const propsData = { value: orderFactory() }
    renderWithMainApp(OrderActionVoid, { propsData })
  ...
  it('should open confirmation dialog with two buttons and close it', async () => {
    const propsData = { value: orderFactory() }
    const { queryByTestId } = renderWithMainApp(OrderActionVoid, { propsData })
  ...
  it('should not emit input event after failed requests, stop loading and trigger error toast', async () => {
    ...
    const wrapper = renderWithMainApp(OrderActionVoid, { propsData })
