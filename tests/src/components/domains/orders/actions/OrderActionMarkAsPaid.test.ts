...
describe('OrderActionMarkAsPaid input component test', () => {
  const attributes = {
    component: 'order-action-mark-as-paid-btn',
  ...
  it('should mount OrderActionMarkAsPaid component correctly', () => {
    const propsData = { value: orderFactory() }
    renderWithMainApp(OrderActionMarkAsPaid, { propsData })
  ...
  it('should not emit input event after failed requests, stop loading and trigger error toast', async () => {
    ...
    const wrapper = renderWithMainApp(OrderActionMarkAsPaid, { propsData })
