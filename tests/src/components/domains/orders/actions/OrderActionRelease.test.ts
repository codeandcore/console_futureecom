...
describe('OrderActionRelease input component test', () => {
  const attributes = {
    component: 'order-action-release-btn',
  ...
  it('should mount OrderActionRelease component correctly', () => {
    const propsData = { value: orderFactory() }
    renderWithMainApp(OrderActionRelease, { propsData })
  ...
  it('should not emit input event after failed requests, stop loading and trigger error toast', async () => {
    ...
    const wrapper = renderWithMainApp(OrderActionRelease, { propsData })
